...
#define BMBT_HEADER_TEMPS 5 // sorry, could not easily include ../ui/bmbt.h but still did not want number in code
...
static void IBusHandleRADMessage(IBus_t *ibus, unsigned char *pkt)
{
    if (pkt[IBUS_PKT_CMD] == IBUS_CMD_MOD_STATUS_RESP) {
        ...
        if ((pkt[IBUS_PKT_CMD] == IBUS_CMD_GT_WRITE_WITH_CURSOR)&&(pkt[IBUS_PKT_CMD+1]==IBUS_CMD_GT_WRITE_ZONE)&&((pkt[IBUS_PKT_CMD+3]==BMBT_HEADER_TEMPS)||(pkt[IBUS_PKT_CMD+3]==(BMBT_HEADER_TEMPS + 0x40)))&&(ConfigGetSetting(CONFIG_SETTING_BMBT_TEMP_HEADERS)!=CONFIG_SETTING_OFF)) {
            if (memchr(pkt+4,0xB0,6)==NULL) {
                // if BB sent this update with temperature ( = has degree char ), do not loop into updating
