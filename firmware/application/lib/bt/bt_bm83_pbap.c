...
//    LogDebug(LOG_SOURCE_BT, "%s", _pb_buffer);
    ...
    // clear the current phone book
    memset(_phonebook, 0, sizeof(_phonebook));
...
void BM83CommandPBAPPullPhoneBook(BT_t *bt, uint8_t type, uint16_t offset)
{
    ...
    uint8_t command[] = {
        ...
        0x54,   // Flags (2 bytes): b14 = max_list_count, b12+b10 = vCard selector
        ...
        (offset & 0xFF),
        0x82,   // vCard selector (8 bytes) - TEL (op) FN
        0x00,
        0x00,
        0x00,
        0x00,
        ...
        0x00,
        0x00,
        0x01    // vCard selector operator (1 byte) - (TEL) AND (FN)
    };
    ...
    switch(event.event_code) {
        ...
        case BM83_PBAP_EVENT_PULL_PHONEBOOK_RSP:
            ...
            if ((event.event_phonebook.flags & FLAGS_PB_DATABASE_ID) > 0) { 
                ...
                for (int i = 0; i < 16; i++) {
                    LogRawDebug(LOG_SOURCE_BT, "%02x", event.event_phonebook.database_id[i]);
                };
                LogRawDebug(LOG_SOURCE_BT, "\r\n");
            }
