...
static void BMBTHeaderWrite(BMBTContext_t *context)
{
    ...
    switch (ConfigGetSetting(CONFIG_SETTING_BMBT_TEMP_HEADERS)) {
...
static void BMBTMenuDashboardUpdate(BMBTContext_t *context, char *f1, char *f2, char *f3)
{
    ...
    if (context->ibus->gtVersion == IBUS_GT_MKIV_STATIC) {
        ...
        char tempUnit = 'C';
        char ambtempstr[11] = {0};
        char oiltempstr[11] = {0};
        char cooltempstr[12] = {0};
        ...
        snprintf(ambtempstr, 11, "A:%d", ambtemp);
        if (cooltemp>0) { 
            snprintf(cooltempstr, 12, "C:%d, ", cooltemp);
        }
        if (oiltemp>0) {
            snprintf(oiltempstr, 11, "O:%d, ", oiltemp);
        };
        char temperature[38]={0};

        snprintf(temperature,38,"Temp %s%s%s\xB0%c",oiltempstr,cooltempstr,ambtempstr,tempUnit);
